...
const SelectArea = (props: SelectAreaProps) => {
  ...
  const { start, center, end } = useMemo(() => {
    ...
    if (contentStatus.selectedRange.start === contentStatus.selectedRange.end) {
      ...
      if (endLineElements.lineElement) {
        ...
        if (hasMultiSubLine) {
          if (multiSubLineStart !== endLine.end) {
            ...
            const centerSubLineRect = centerSubLineRange.getBoundingClientRect()
      ...
      if (endhasMultiSubLine && startHasMultiSubLine) {
        ...
        const endLineId = contentStatus.ids[contentStatus.selectedRange.end - 1]
